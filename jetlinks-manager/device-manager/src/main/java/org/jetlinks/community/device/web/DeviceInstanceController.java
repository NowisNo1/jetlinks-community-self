...
public class DeviceInstanceController implements
    ...
    @PostMapping("/list/{id:.+}")
    @QueryAction
    @Operation(summary = "根据产品Id进行筛选")
    public Flux<DeviceInstanceEntity> getInstanceByProductId(@PathVariable @Parameter(description = "产品ID") String id) {
    ...
    @QueryAction
    @Operation(summary = "根据设备名称进行筛选")
    public Flux<DeviceInstanceEntity> getInstanceByName(@PathVariable @Parameter(description = "设备名称") String name) {
        return instanceRepository
            .createQuery()
